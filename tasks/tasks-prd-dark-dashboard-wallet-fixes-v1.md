...
- [ ] 1.0 Fix wallet onboarding reliability and provider prerequisites first (highest risk)
  - [ ] 1.1 Make wallet creation success independent from initial scan success and return onboarding hints
    - covers: FR-5, FR-6
  ...
  - [ ] 1.2 Add wallet onboarding status endpoint and recovery wiring
    - covers: FR-8
  ...
  - [ ] 1.3 Implement provider policy: CoinGecko required, Birdeye optional
    - covers: FR-11
  ...
  - [ ] 1.4 Make CoinGecko client runtime-compatible with configured base URL/key mode
    - covers: FR-12
...
- [ ] 2.0 Add backend data surfaces for dashboard visibility
  - [ ] 2.1 Implement `/api/portfolio/dashboard` aggregate endpoint
    - covers: FR-4
  ...
  - [ ] 2.2 Add repository helpers to fetch grouped latest snapshot asset/protocol rows and totals
    - covers: FR-2, FR-3
  ...
  - [ ] 2.3 Extend daily snapshots to include protocol-position items with unknown-value semantics
    - covers: FR-9, FR-10
  ...
  - [ ] 2.4 Make snapshot protocol-read failures degrade to partial rather than hard fail
    - covers: FR-15
...
- [ ] 3.0 Deliver UI behavior changes (dark mode + dashboard + onboarding feedback)
  - [ ] 3.1 Convert web experience to dark-only visual system
    - covers: FR-1
  ...
  - [ ] 3.2 Upgrade dashboard to show totals and grouped token/protocol tables from new endpoint
    - covers: FR-2, FR-3
  ...
  - [ ] 3.3 Improve settings wallet add flow with explicit outcome panel, recent wallets, and recovery actions
...
- [ ] 4.0 Lock test strategy and CI behavior
  - [ ] 4.1 Add deterministic API/UI coverage for wallet onboarding and dashboard balances
    - covers: FR-13, FR-15
  ...
  - [ ] 4.2 Enforce deterministic CI gate excluding `@live` tests
    - covers: FR-13
  ...
  - [ ] 4.3 Maintain operator-run live smoke suite as separate path
    - covers: FR-14
...
- [ ] 5.0 Final hardening and release-readiness checks
  - [ ] 5.1 Add regression checks for provider error messaging and recovery hints in UI text
  ...
  - [ ] 5.2 Validate dashboard behavior with empty-state and partial-snapshot cases
