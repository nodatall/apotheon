...
- `apps/api/src/db/repositories/tracked-tokens.repository.js` - Shared tracked-token upsert/list operations for scan + manual token flows.
- `apps/api/src/db/run-migrations.js` - Migration runner with applied-migration tracking for rerunnable DB setup.
...
- `apps/api/src/routes/wallets.test.js` - Wallet route validation tests for chain/address safety and duplicate handling.
- `apps/api/src/routes/history.test.js` - Portfolio history route contract test.
- `apps/api/src/routes/jobs-status.test.js` - Job-status endpoint tests across universe, wallet scan, and snapshots routes.
...
- `apps/api/src/services/wallet-scan/wallet-scan-trigger.test.js` - Auto scan trigger/snapshot binding tests on wallet onboarding.
- `apps/api/src/services/wallet-scan/wallet-rescan.test.js` - Manual rescan idempotency/traceability tests.
- `apps/api/src/services/wallet-scan/auto-track.test.js` - Auto-track behavior tests for positive-balance assets.
- `apps/api/src/services/wallet-scan/wallet-scan-batching.test.js` - Batch balance resolution tests for scan performance guardrails.
- `apps/api/src/services/tokens/manual-token.test.js` - Manual token registration tests with metadata override coverage.
- `apps/api/src/services/protocols/protocols.test.js` - Protocol contract persistence/validation-status tests.
- `apps/api/src/services/protocols/abi-mapping.test.js` - ABI mapping schema and preview validation tests.
- `apps/api/src/services/valuation/valuation.test.js` - Unknown valuation preservation tests.
- `apps/api/src/services/snapshots/snapshot-batching.test.js` - Snapshot valuation batching tests.
- `apps/api/src/jobs/snapshots-scheduler.test.js` - UTC scheduler idempotency + rerun tests.
- `apps/api/src/test/run-tests.js` - Filterable Node test runner used by task verify commands.
- `apps/web/src/App.jsx` - Single-owner application shell and page navigation.
- `apps/web/src/styles.css` - Responsive visual system and component/page styling.
...
- `package.json` - Root scripts/dependencies including deterministic and live Playwright commands.
...
- [x] 2.0 Implement wallet onboarding and universe-based scanning
  - [x] 2.1 Implement wallet add/list with chain-aware address validation
    - covers: FR-3
  ...
  - [x] 2.2 Trigger scan automatically on wallet creation with immutable universe snapshot reference
    - covers: FR-4
  ...
  - [x] 2.3 Implement manual re-scan endpoint and idempotent state update
    - covers: FR-5, FR-18
  ...
  - [x] 2.4 Implement auto-tracking for held universe tokens
    - covers: FR-10
  ...
  - [x] 2.5 Implement batched wallet balance resolution path
    - covers: FR-22
...
- [x] 3.0 Implement manual token and protocol contract extension paths
  - [x] 3.1 Implement manual token registration with auto metadata + override persistence
    - covers: FR-11
  ...
  - [x] 3.2 Implement protocol contract registration model and endpoints
    - covers: FR-12
  ...
  - [x] 3.3 Implement ABI mapping validator with preview call requirement
    - covers: FR-13
...
- [x] 4.0 Implement daily snapshots, valuation resilience, and history APIs
  - [x] 4.1 Implement UTC daily snapshot scheduler and manual rerun endpoint
    - covers: FR-14
  ...
  - [x] 4.2 Implement valuation pipeline with unknown-value preservation
    - covers: FR-15
  ...
  - [x] 4.3 Implement portfolio history query APIs
    - covers: FR-16
  ...
  - [x] 4.4 Implement job status and error visibility endpoints
    - covers: FR-18
  ...
  - [x] 4.5 Implement batched snapshot valuation and aggregation path
    - covers: FR-22
...
- [x] 5.0 Implement required frontend surfaces and operational UX
  - [x] 5.1 Build required pages and API integrations (Dashboard, Assets, Protocols, History, Settings)
  ...
  - [x] 5.2 Add single-owner app shell constraints and non-auth operation UX
    - covers: FR-1
...
- [x] 6.0 Enforce end-to-end quality gates and traceability
  - [x] 6.1 Implement deterministic Playwright E2E suite, mocks/fixtures, and story traceability matrix
    - covers: FR-19, FR-24
  ...
  - [x] 6.2 Add CI merge gate for deterministic suites and enforce live-test exclusion
    - covers: FR-20, FR-24
  ...
  - [x] 6.3 Add operator-run live smoke E2E suite and command
    - covers: FR-25
  ...
  - [x] 6.4 Emit machine-readable artifact bundle for live smoke runs
    - covers: FR-26
